...
const EditableValue = ({ value, onChange, min, max, prefix = "", suffix = "" }: {
  value: number; onChange: (v: number) => void; min: number; max: number; prefix?: string; suffix?: string;
}) => {
...
const GrowthCalculator = ({ onCtaClick }: GrowthCalculatorProps) => {
  ...
  const results = [
    { label: "People Who Reply", value: replies.toLocaleString(), icon: MessageSquare, color: "text-primary" },
    { label: "Calls Booked", value: bookings.toLocaleString(), icon: CalendarCheck, color: "text-primary" },
    { label: "Money You Could Make", value: `$${revenue.toLocaleString()}`, icon: DollarSign, color: "text-success", highlight: true },
  ];
    ...
    <section className="py-20 md:py-28">
      <div className="container mx-auto px-4 max-w-4xl">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-foreground mb-4">How Much Money Could You Make?</h2>
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">
            Move the sliders or click the numbers to type. See how many clients and how much money you could get every month.
          </p>
        </motion.div>
          ...
          <CardContent className="p-8 md:p-10 space-y-10">
            <div>
              <div className="flex justify-between items-end mb-4">
                <label className="text-sm font-medium text-foreground">How many emails we send for you per month</label>
                <EditableValue value={volume} onChange={setVolume} min={500} max={500000} />
              </div>
            ...
            <div>
              <div className="flex justify-between items-end mb-4">
                <label className="text-sm font-medium text-foreground">How much is one sale worth to you?</label>
                <EditableValue value={dealValue} onChange={setDealValue} min={1000} max={50000} prefix="$" />
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button onClick={onCtaClick} size="lg" className="rounded-2xl px-8 py-6 text-base font-semibold shadow-lg shadow-primary/25 bg-gradient-to-r from-primary to-primary/80">
                I Want These Results <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
              <Button onClick={onCtaClick} size="lg" variant="outline" className="rounded-2xl px-8 py-6 text-base font-semibold border-primary/20">
                <Rocket className="mr-2 h-5 w-5" /> Let&apos;s Talk — Free Call
              </Button>
            </div>
          </CardContent>
        </Card>
